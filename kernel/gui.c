...
/**
 * FUNGSI DRAWICON YANG DIPERBAIKI
 * Mengabaikan pixel hitam pekat (0xFF000000) yang ada di icons_data.c
 */
int drawIcon(RGB *buf, int x, int y, int icon, RGBA color) {
	...
	for (i = 0; i < ICON_SIZE; i++) {
		...
		for (j = 0; j < ICON_SIZE; j++) {
			...
			/* * PERBAIKAN: Melewati (skip) penggambaran jika pixel
			 * adalah:
			 * 1. 0xFF000000 (Hitam pekat yang digunakan di
			 * icons_data.c Anda)
			 * 2. 0x00000000 (Transparan murni)
			 * 3. ICON_TRANSPARENT (Sesuai definisi di icons.h)
			 */
