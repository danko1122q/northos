...
void renderContent(void) {
    ...
    addColorFillWidget(&startWindow, bgColor, 
                       0, HEADER_HEIGHT, 
                       MENU_WIDTH, MENU_HEIGHT - HEADER_HEIGHT, 
                       0, scrollHandler);
    ...
    for (i = 0; i < PROGRAM_COUNT; i++) {
        ...
        if (currentY + BUTTON_HEIGHT >= HEADER_HEIGHT && currentY < MENU_HEIGHT) {
            addButtonWidget(&startWindow, textColor, appBgColor, 
                            programs[i].display,
                            BUTTON_PADDING_X, currentY, 
                            MENU_WIDTH - (BUTTON_PADDING_X * 2) - SCROLLBAR_WIDTH - 5, 
                            BUTTON_HEIGHT,
                            0, startProgramHandler);
        }
    ...
    if (currentY >= HEADER_HEIGHT && currentY < MENU_HEIGHT) {
        addColorFillWidget(&startWindow, dividerColor, 
                           BUTTON_PADDING_X, currentY, 
                           MENU_WIDTH - (BUTTON_PADDING_X * 2) - SCROLLBAR_WIDTH - 5, 2, 
                           0, emptyHandler);
    }
    ...
    if (currentY >= HEADER_HEIGHT && currentY < MENU_HEIGHT) {
        addButtonWidget(&startWindow, textColor, bgColor, "System",
                        BUTTON_PADDING_X, currentY, 80, 25, 0, emptyHandler);
    }
    
    currentY += 35;
    
    // System buttons
    if (currentY >= HEADER_HEIGHT && currentY + BUTTON_HEIGHT < MENU_HEIGHT) {
        int btnWidth = (MENU_WIDTH - (BUTTON_PADDING_X * 3) - SCROLLBAR_WIDTH - 5) / 2;
        
        addButtonWidget(&startWindow, textColor, dangerColor, "Restart",
                        BUTTON_PADDING_X, currentY, btnWidth, BUTTON_HEIGHT,
                        0, rebootHandler);
        
        addButtonWidget(&startWindow, textColor, dangerColor, "Shutdown",
                        BUTTON_PADDING_X + btnWidth + BUTTON_PADDING_X, currentY,
    ...
    addColorFillWidget(&startWindow, scrollbarBg,
                       ...
                       SCROLLBAR_WIDTH, MENU_HEIGHT - HEADER_HEIGHT,
                       0, emptyHandler);
    
    // Scrollbar thumb (if scrollable)
    if (maxScrollOffset > 0) {
        int visibleHeight = MENU_HEIGHT - HEADER_HEIGHT;
        int thumbHeight = (visibleHeight * visibleHeight) / contentHeight;
        if (thumbHeight < 20) thumbHeight = 20;
        
        int thumbPosition = (scrollOffset * (visibleHeight - thumbHeight)) / maxScrollOffset;
        
        addColorFillWidget(&startWindow, scrollbarThumb,
                           MENU_WIDTH - SCROLLBAR_WIDTH - 2, 
                           HEADER_HEIGHT + thumbPosition,
                           SCROLLBAR_WIDTH, thumbHeight,
                           0, emptyHandler);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    contentHeight = HEADER_HEIGHT + 
                    ...
                    SECTION_SPACING + 2 + SECTION_SPACING + 25 + 35 + BUTTON_HEIGHT + 20;
    ...
    addColorFillWidget(&startWindow, headerBg, 
                       0, 0, MENU_WIDTH, HEADER_HEIGHT, 0, emptyHandler);
    
    addButtonWidget(&startWindow, textColor, headerBg, "Programs",
                    BUTTON_PADDING_X, BUTTON_PADDING_Y, 100, 30, 0, emptyHandler);
    ...
    while (1) {
        ...
        if (startWindow.needsRepaint) {
            ...
            for (i = startWindow.widgetlisthead; i != -1; ) {
                ...
                // Keep only first 2 widgets (header bg and title)
                if (i > 1) {
