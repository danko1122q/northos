...
Widget* getInputWidget() {
    int i;
    for (i = editorWindow.widgetlisthead; i != -1; i = editorWindow.widgets[i].next) {
...
void safeKeyHandler(Widget *w, message *msg) {
    ...
    // Up arrow (main keyboard or numpad 8) - move to previous line
        ...
        if (lineStart > 0) {
            ...
            int prevLineEnd = lineStart - 1; // Position of \n before current line
            ...
            if (col > prevLineLen) col = prevLineLen;
    ...
    // Down arrow (main keyboard or numpad 2) - move to next line
        ...
        if (lineEnd < len) {
            ...
            if (col > nextLineLen) col = nextLineLen;
...
void initEditor(char *file) {
    ...
    // Background
    addColorFillWidget(&editorWindow, createColor(250, 250, 250, 255), 
                       0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, 0, emptyHandler);
    
    // Toolbar background
    addColorFillWidget(&editorWindow, createColor(240, 240, 240, 255), 
                       0, 0, WINDOW_WIDTH, TOOLBAR_HEIGHT, 0, emptyHandler);
    ...
    addInputFieldWidget(&editorWindow, createColor(40, 40, 40, 255), text,
                        EDITOR_X, EDITOR_Y, 
                        WINDOW_WIDTH - EDITOR_X * 2, editorHeight,
                        0, editorInputHandler);
    
    // Save button
    addButtonWidget(&editorWindow, createColor(255, 255, 255, 255), 
                    createColor(76, 175, 80, 255), "Save",
